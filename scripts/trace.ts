...
/**
 * Read scenarios + decision-log + every source file under SOURCE_DIRS
 * and assemble a TraceGraph. `repoRoot` defaults to the script's
 * resolved REPO_ROOT, but can be overridden so tests can drive the
 * full pipeline against a synthetic repo layout (e.g. a tmp dir
 * shaped like `.claude/worktrees/<name>/`) without spawning a
 * subprocess.
 */
export function loadGraphFromDisk(repoRoot: string = REPO_ROOT): TraceGraph {
  const scenariosMd = readFileSync(join(repoRoot, SCENARIOS_PATH), 'utf-8');
  const decisionLogMd = readFileSync(join(repoRoot, DECISION_LOG_PATH), 'utf-8');
  ...
  for (const dir of SOURCE_DIRS) {
    ...
    let absPaths: string[];
    try {
      absPaths = walkSource(fullDir, repoRoot);
    } catch {
      continue;
    }
